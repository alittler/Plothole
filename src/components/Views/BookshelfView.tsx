...
interface BookshelfViewProps {
  ...
  onCreateProject: (title: string, author: string, useSample: boolean) => void;
...
export const BookshelfView: React.FC<BookshelfViewProps> = ({
  ...
  const handleCreate = () => {
    ...
    onCreateProject(newTitle, newAuthor || 'Unknown Author', false);
    setIsCreating(false);
    setNewTitle('');
    setNewAuthor('');
  };
    ...
    <div className="h-full overflow-y-auto p-4 md:p-8 bg-slate-50 dark:bg-slate-950">
      <div className="max-w-6xl mx-auto space-y-8 md:space-y-12">
        <header className="space-y-2 text-center md:text-left">
          <h1 className="text-3xl md:text-4xl font-black tracking-tight text-slate-900 dark:text-white">MY LIBRARY</h1>
          <p className="text-sm md:text-base text-slate-500 dark:text-slate-400">Manage your story worlds and manuscripts.</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          {projects.map(project => (
            <div
              key={project.id}
              className="h-64 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col group hover:shadow-md transition-all"
            >
              <div className="flex-1 p-6 flex flex-col justify-between">
                <div>
                  <h3 className="font-black text-xl text-slate-900 dark:text-white line-clamp-1">{project.title}</h3>
